...
use bevy_asset::{Asset, AssetRef, AssetServer, Assets};
use bevy_ecs::{
    bundle::Bundle,
    error::Result,
    event::EntityEvent,
...
impl SceneDependencies {
    ...
    pub fn register_erased(&mut self, type_id: TypeId, path: AssetRef<'static>) {
        self.0.push(SceneDependency { path, type_id });
    }

    /// Registers a new asset dependency with the given `A` type and `path`. `A` should match
    /// the type of the asset being loaded.
    pub fn register<A: Asset>(&mut self, path: AssetRef<'static>) {
...
pub enum ResolveSceneError {
    ...
    MissingSceneDependency(AssetRef<'static>),
...
pub struct InheritSceneAsset(
    /// The [`AssetRef`] of the [`ScenePatch`] to inherit from.
    pub AssetRef<'static>,
);

// XXX TODO: This was changed from AssetRef to AssetPath, but we might need a parallel AssetPath impl?
impl<I: Into<AssetRef<'static>>> From<I> for InheritSceneAsset {
