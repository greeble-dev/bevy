...
impl AssetServer {
    ...
    pub(crate) fn load_with_meta_transform<'a, G: Send + Sync + 'static>(
        ...
        // XXX TODO: See comment on settings parameter.
        if let Some(settings) = settings {
            path = path.with_settings(settings);
        }

        if let Err(err) = validate_ref(&path, override_unapproved, self.data.unapproved_path_mode) {
    ...
    pub(crate) fn load_unknown_type_with_meta_transform<'a, G: Send + Sync + 'static>(
        ...
        let (handle, should_load) = infos.get_or_create_path_handle::<LoadedUntypedAsset>(
            path.clone().with_source(untyped_source).into(),
            HandleLoadingMode::Request,
            meta_transform,
        );
    ...
    pub fn reload<'a>(&self, path: impl Into<AssetRef<'a>>) {
        self.reload_internal(path, false);
    }

    fn reload_internal<'a>(&self, path: impl Into<AssetRef<'a>>, log: bool) {
        let server = self.clone();
        let path = path.into().into_owned();
        IoTaskPool::get()
            .spawn(async move {
            ...
            .spawn(async move {
                ...
                match load_folder(source.id(), path.path(), asset_reader, &server, &mut handles).await {
                    ...
                    Err(err) => {
                        ...
                        server.send_asset_event(InternalAssetEvent::Failed { index, error: err, path: path.into() });
                    },
                }
            })
            .detach();
    ...
    /// or is still "alive".
    pub fn get_handle<'a, A: Asset>(&self, path: impl Into<AssetRef<'a>>) -> Option<Handle<A>> {
    ...
    pub fn get_path_id<'a>(&self, path: impl Into<AssetRef<'a>>) -> Option<UntypedAssetId> {
        let infos = self.read_infos();
        let path = path.into();
    ...
    pub fn get_path_ids<'a>(&self, path: impl Into<AssetRef<'a>>) -> Vec<UntypedAssetId> {
        let path = path.into();
        self.read_infos()
    ...
    pub fn get_handle_untyped<'a>(&self, path: impl Into<AssetRef<'a>>) -> Option<UntypedHandle> {
        let path = path.into();
    ...
    pub fn get_handles_untyped<'a>(&self, path: impl Into<AssetRef<'a>>) -> Vec<UntypedHandle> {
        let path = path.into();
    ...
    pub fn get_path(&self, id: impl Into<UntypedAssetId>) -> Option<AssetRef<'_>> {
        let Ok(index) = id.into().try_into() else {
            // Always say we don't have Uuid assets.
            return None;
        };
...
impl<'a> LoadBuilder<'a> {
    ...
    // `load_untyped_async` on the other hand lets you avoid dealing with the "missing type" of the
    // asset (i.e., dealing with `LoadedUntypedAsset`).
    ...
    pub async fn load_untyped_async<'b>(
        ...
        self.asset_server
            // XXX TODO: Work out what to do with the meta transform.
            //.load_internal(None, path.into(), false, self.meta_transform)
            .load_internal(None, path.into(), false, None)
            .await
...
pub fn handle_internal_asset_events(world: &mut World) {
    world.resource_scope(|world, server: Mut<AssetServer>| {
        ...
        fn queue_ancestors(
            asset_path: &AssetRef,
            infos: &AssetInfos,
            paths_to_reload: &mut HashSet<AssetRef<'static>>,
        ) {
...
fn validate_ref(
    ...
    if let Some(path) = path.try_temporary_path_workaround() {
        validate_path(&path, override_unapproved, unapproved_path_mode)
    } else {
        Ok(())
    }
}
