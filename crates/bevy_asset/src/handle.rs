use crate::{
    meta::MetaTransform, Asset, AssetId, AssetIndex, AssetIndexAllocator, AssetRef, AssetServer,
...
pub enum HandleTemplate<T: Asset> {
    /// Creates a [`Handle`] by calling [`AssetServer::load`] on the given [`AssetRef`].
...
impl<I: Into<AssetRef<'static>>, T: Asset> From<I> for HandleTemplate<T> {
    fn from(value: I) -> Self {
        Self::Path(value.into())
    }
}
...
mod tests {
    ...
    use crate::{tests::create_app, AssetApp, AssetDependency, Assets, VisitAssetDependencies};
