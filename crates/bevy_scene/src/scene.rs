...
/// A [`Scene`] that will include the cached [`ScenePatch`] stored at the given [`AssetRef`].
...
pub struct CachedSceneAsset(
    /// The [`AssetRef`] of the [`ScenePatch`] to include.
    pub AssetRef<'static>,
);

impl<I: Into<AssetRef<'static>>> From<I> for CachedSceneAsset {
